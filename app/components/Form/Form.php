...
abstract class Form
{
    ...
    {
        $errorsString = 'Ошибки' . PHP_EOL;
        foreach ($this->getErrors() as $fieldName => $errors) {
            $errors = array_map(static function ($item) {
                return trim($item, '. \t\n\r\0\x0B');
            }, $errors);
            $errorsString .= 'Поле "' . $fieldName . '": ' . implode('. ', $errors) . PHP_EOL;
        }
        return $errorsString;
    }
    ...
    public function getErrorsMessages(): array
    {
        ...
        foreach ($this->getErrors() as $fieldName => $errors) {
            ...
            $errorsMessages[] = 'Поле "' . $fieldName . '": ' . implode('. ', $errors);
        }
